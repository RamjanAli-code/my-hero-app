...
const Description = () => {
  ...
  const isInstalled = existing.some(item => item.id === data.id);
        ...
        <div className='flex flex-col h-2/3 w-2/3  md:w-6/12'>
          ...
          <div className="flex  items-start justify-start gap-4 mb-4 h-2/3 pt-2 pb-2 md:pt-10 md:pb-10">
            ...
            <div className=" w-8/12 md:w-9/12  flex flex-col items-start justify-start rounded-lg">
              ...
              <h1 className="text-xl font-bold text-black">{data.reviews}</h1>
            </div>
          </div>
          <button
            onClick={install} className={`btn w-6/12 ${installed ? 'btn-primary cursor-not-allowed' : 'btn-primary'}`}>
